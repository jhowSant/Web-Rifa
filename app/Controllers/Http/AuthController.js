...
class AuthController {
  async login({ request, auth, response, session }) {
    ...
    const usuarios = await Persona.verify(payload);

    await auth.login(usuarios);
  ...
  async register({ request, auth, response, session }) {
    ...
    const usuarios = await Persona.register(payload);
  ...
  async forgotPassword({ request, response, session, auth }) {
    ...
    try {
      const usuarios = await Persona.updatePasswordByToken(token, payload);
